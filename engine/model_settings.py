...
from engine.pipeline import DATA_DIR
...
LEGACY_SERVER_A_URL = "http://10.0.11.83:8011/v1"
LEGACY_SERVER_A_MODEL = "malapp-model-a"
...
def default_model_settings() -> dict[str, Any]:
    return {
        "server_models_enabled": _env_enabled("MALAPP_USE_SERVER_MODELS", "1"),
...
def load_model_settings_without_apply() -> dict[str, Any]:
    ...
    if isinstance(saved, dict):
        ...
        if (
            settings.get("model_a_api_url") == LEGACY_SERVER_A_URL
            and settings.get("model_a_model") == LEGACY_SERVER_A_MODEL
        ):
            settings["model_a_api_url"] = DEFAULT_SERVER_A_URL
            settings["model_a_model"] = DEFAULT_SERVER_A_MODEL
            settings["model_a_api_key"] = settings.get("model_a_api_key") or DEFAULT_SERVER_A_API_KEY
        if _env_enabled("MALAPP_FORCE_TUNNEL_MODELS", "0"):
            settings["server_models_enabled"] = True
            settings["model_a_api_url"] = os.getenv("MALAPP_MODEL_A_API_URL", settings["model_a_api_url"])
            settings["model_b_api_url"] = os.getenv("MALAPP_MODEL_B_API_URL", settings["model_b_api_url"])
            settings["model_a_model"] = os.getenv("MALAPP_MODEL_A_MODEL", settings["model_a_model"])
            settings["model_b_model"] = os.getenv("MALAPP_MODEL_B_MODEL", settings["model_b_model"])
            settings["model_a_api_key"] = os.getenv("MALAPP_MODEL_A_API_KEY", settings.get("model_a_api_key") or DEFAULT_SERVER_A_API_KEY)
            settings["model_b_api_key"] = os.getenv("MALAPP_MODEL_B_API_KEY", settings.get("model_b_api_key") or "")
    return settings
...
def ensure_runtime_ready_for_judgement() -> dict[str, Any]:
    ...
    if mode == "server_models_unavailable":
        ...
        raise RuntimeError(
            ...
            "请在 83 服务器上确认 vLLM/OpenAI-compatible 服务已启动、"
            "8011/8012 端口已监听并允许本机访问，模型名与 API 密钥和 APP 配置一致。"
        )
