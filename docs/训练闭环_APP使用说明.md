...
- 执行信息：是否缓存、是否 Hermes 编排、模型签名、错误信息。

相关代码：

- `engine/agent_trace.py`
- `engine/pipeline.py` 中 `judge()` 结束时调用 `save_agent_trace(report)`
- `run.py` 中 `/api/agent-trace` 和 `/api/agent-traces`
...
- `engine/agent_trace.py` 中 `save_human_review()`
- `run.py` 中 `POST /api/human-reviews`
...
- `engine/reward_builder.py`
- `engine/pipeline.py` 中 `save_reward_for_report(report)`
- `run.py` 中 `/api/rewards`
...
.\.venv\Scripts\python.exe tools\export_training_datasets.py --limit 5000
```
...
- `engine/dataset_export.py`
- `run.py` 中 `POST /api/datasets/export`
- `tools/export_training_datasets.py`
...
```powershell
cd C:\Users\啤酒肚\Desktop\工作\test1
.\.venv\Scripts\python.exe ml_pipeline\train_policy_model.py data\exports\training_loop_YYYYMMDD_HHMMSS\policy_training.jsonl
```
...
- `ml_pipeline/train_policy_model.py`
