...
from ml_pipeline import xgb_pipeline as xp
...
def fit_model(rows: list[dict[str, Any]], features: list[str], name: str, model_dir: Path, curve_dir: Path, train_split: str = "train"):
    ...
    metrics = {
        ...
        "feature_importance": sorted(zip(features, model.feature_importances_.tolist()), key=lambda item: item[1], reverse=True),
...
def export_rows(out_dir: Path, heldout: list[dict[str, Any]], wec_rows: list[dict[str, Any]], wec, wec_features: list[str], thresholds: dict[str, Any]) -> None:
    by_md5 = {row["md5"]: row for row in wec_rows if row["split"] == "test"}
    app_headers = list(read_workbook(Path(args_for_export["malicious"]))[0].keys()) if False else None
    output_rows = []
    ...
    prob_by_md5 = {row["md5"]: float(prob) for row, prob in zip(test_rows, probabilities)}
    for item in heldout:
        raw = dict(item["raw"])
