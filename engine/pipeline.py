...
from engine.agent_runtime import AgentSpec, run_agent_cluster
from engine.business_label import analyze_business_label
from engine.debate_flow import run_debate
from engine.decision_engine import collaborative_decision
from engine.evidence_layers import (
    build_llm_explanation_layer,
    build_raw_evidence_layer,
    build_structured_evidence_layer,
)
from engine.llm_local import local_qwen_enabled, normalize_llm_result, parse_model_json, qwen_generate
from engine.impersonation import analyze_impersonation
from engine.rag import rag_context_for_sample
from engine.static_features import analyze_apk_from_sample, public_static_feedback
from engine.threat_intelligence import analyze_threat_intelligence
...
def insert_report(report: dict[str, Any]) -> None:
    ...
    try:
        ...
        conn.close()
    try:
        from engine.agent_trace import init_trace_tables
        from engine.reward_builder import init_reward_tables
...
def normalize_sample(raw: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    bundled_mapping_path = Path(__file__).resolve().parents[1] / "data" / "field_mapping.json"
    mapping = (
    ...
    from engine.preprocess import derive_engine_fields
...
def static_analysis_agent(sample: dict[str, Any]) -> EvidenceBlock:
    ...
    missing: list[str] = []
    score = 0.25
    apk_analysis = sample.get("apk_analysis") if isinstance(sample.get("apk_analysis"), dict) else {}
    sdk_risk = sample.get("sdk_risk") if isinstance(sample.get("sdk_risk"), dict) else {}
    static_trust = sample.get("static_trust") if isinstance(sample.get("static_trust"), dict) else {}
    conflict = sample.get("engine_conflict") if isinstance(sample.get("engine_conflict"), dict) else {}

    if sample.get("signature_status") in {"tampered", "mismatch", "invalid"}:
        evidence.append("签名状态显示存在篡改、错配或无效风险。")
        score += 0.35
    elif sample.get("signature_status") == "missing":
        evidence.append("APK 未发现 META-INF 证书签名文件，签名可信度异常。")
        score += 0.3
    elif sample.get("signature_status") in {"valid", "normal"}:
        evidence.append("签名状态正常，静态签名侧暂未发现明显异常。")
        score -= 0.08
    else:
        missing.append("signature_status")

    permissions = {str(x).lower() for x in sample.get("permissions", [])}
    risky = sorted(p for p in permissions if any(k in p for k in ["sms", "contact", "overlay", "install", "accessibility"]))
    if risky:
        evidence.append(f"发现高风险 Android 权限：{', '.join(risky[:5])}。")
        score += min(0.25, 0.06 * len(risky))
    packer_value = sample.get("packer")
    packer_detected = (
        packer_value
        if isinstance(packer_value, bool)
        else str(packer_value or "").strip().lower()
        not in {"", "0", "false", "no", "none", "否", "未加固", "无壳", "未检测到"}
    )
    if packer_detected:
        evidence.append("样本存在加固、壳或混淆标记，需要提高静态分析风险权重。")
        score += 0.15
    packer_matches = sample.get("packer_matches") or apk_analysis.get("packer", {}).get("matches") or []
    if packer_matches:
        names = ", ".join(str(item.get("name", item)) for item in packer_matches[:5])
        evidence.append(f"加固壳指纹命中：{names}，存在脱壳特征隐藏风险。")
        score += 0.18
    sdk_summary = sdk_risk.get("risk_summary", {})
    if sdk_summary.get("high") or sdk_summary.get("medium"):
        evidence.append(
            f"第三方 SDK 风险：高风险 {sdk_summary.get('high', 0)} 个，中风险 {sdk_summary.get('medium', 0)} 个。"
        )
        score += min(0.18, 0.08 * sdk_summary.get("high", 0) + 0.04 * sdk_summary.get("medium", 0))
    if static_trust:
        evidence.append(f"静态可信度评分 {static_trust.get('score', '-')}/100，等级 {static_trust.get('level', '-')}。")
        if static_trust.get("level") == "low":
            score += 0.18
        elif static_trust.get("level") == "medium":
            score += 0.08
        elif static_trust.get("level") == "high":
            score -= 0.04
    if conflict:
        gap = conflict.get("score_gap")
        label_a = conflict.get("engine_a_label") or "unknown"
        label_b = conflict.get("engine_b_label") or "unknown"
        evidence.append(
            f"上游引擎结论存在差异：引擎甲为 {label_a}，引擎乙为 {label_b}"
            + (f"，评分差 {float(gap):.0f} 分。" if gap is not None else "。")
        )
        if gap is not None and float(gap) >= 60:
            score += 0.12
            missing.append("apk_static_features_for_conflict_resolution")
    if not evidence:
        evidence.append("当前字段中未发现明确的静态高危信号。")

    score = clamp(score)
    return EvidenceBlock("static_analysis", label_claim(score, "静态特征风险"), evidence, score, missing, score)


def threat_intel_agent(sample: dict[str, Any], iocs: list[dict[str, Any]]) -> EvidenceBlock:
    evidence: list[str] = []
    score = 0.2
    threat_report = sample.get("threat_intelligence") if isinstance(sample.get("threat_intelligence"), dict) else {}
    threat_block = threat_report.get("evidence_block", {}) if threat_report else {}
    if threat_block:
        evidence.extend(str(item) for item in threat_block.get("evidence", []))
        score = max(score, float(threat_block.get("score", 0.0)))

    suspicious_terms = ("c2", "bot", "fraud", "phish", "malware", "black", "risk")
    field_names = {
        "control_url": "控制端地址",
        "download_url": "下载地址",
    ...
    for key in ("control_url", "download_url", "control_mailbox", "control_phone"):
        value = str(sample.get(key, "")).lower()
        if any(term in value for term in suspicious_terms):
            evidence.append(f"{field_names[key]}包含高风险关键词。")
            score += 0.22

    if iocs:
        evidence.append(f"共提取 {len(iocs)} 个网络威胁指标。")
        score += min(0.25, len(iocs) * 0.04)

    family = str(sample.get("fraud_family", "")).strip()
    if family:
        family_name = {
            "phishing_wallet_family": "钓鱼钱包黑产家族",
        }.get(family, family.replace("_", " "))
        evidence.append(f"样本与已知黑产家族“{family_name}”存在关联。")
        score += 0.18
    virus_name = str(sample.get("virus_name", "")).strip()
    if virus_name and virus_name.lower() not in {"unknown", "未知"}:
        evidence.append(f"上游恶意引擎给出的病毒家族标识为“{virus_name}”。")
        lowered = virus_name.lower()
        if any(term in lowered for term in ("fraud", "rogue", "trojan", "spy", "phish")):
            score += 0.12

    missing = list(threat_block.get("missing_fields", []))
    if not any(sample.get(k) for k in ("control_url", "download_url", "control_mailbox", "control_phone")):
        missing.append("network_ioc_fields")
    if not evidence:
        evidence.append("当前数据中未发现明确的情报命中。")
    score = clamp(score)
    return EvidenceBlock("threat_intel", label_claim(score, "情报关联风险"), evidence, score, sorted(set(missing)), score)
...
def impersonation_agent(sample: dict[str, Any]) -> EvidenceBlock:
    ...
    evidence_block = analysis.get("evidence_block", {}) if analysis else {}
    if evidence_block:
        evidence.extend(str(item) for item in evidence_block.get("evidence", []))
        score = max(score, float(evidence_block.get("score", 0.0)))

    fake_app = sample.get("fake_app")
    if str(fake_app).lower() in {"1", "true", "yes", "y"}:
        evidence.append("样本存在仿冒应用标记。")
        score += 0.18

    app_name = str(sample.get("app_name", "")).lower()
    package_name = str(sample.get("package_name", "")).lower()
    brand_names = {
        "bank": "银行",
        "wallet": "钱包",
        "pay": "支付",
    ...
    for brand in ("bank", "wallet", "pay", "loan", "secure", "finance"):
        if brand in app_name or brand in package_name:
            evidence.append(f"应用名称或包名包含“{brand_names[brand]}”相关敏感词。")
            score += 0.05
    if app_name:
        evidence.append(f"当前样本应用名称为“{sample.get('app_name')}”，已纳入仿冒语义检查。")

    similarity = sample.get("brand_similarity")
    try:
        similarity_score = float(similarity)
    except (TypeError, ValueError):
        similarity_score = None
    if similarity_score is not None and similarity_score >= 0.75:
        evidence.append(f"与正版应用特征的相似度为 {similarity_score:.2f}。")
        score += 0.12

    missing = list(evidence_block.get("missing_fields", []))
    if not evidence:
        evidence.append("当前数据中未发现足够强的仿冒证据。")
    score = clamp(score)
    return EvidenceBlock("impersonation", label_claim(score, "仿冒风险"), evidence, score, sorted(set(missing)), score)

def business_label_agent(sample: dict[str, Any]) -> EvidenceBlock:
    evidence: list[str] = []
    score = 0.15
    labels = []
    analysis = sample.get("business_label_analysis") if isinstance(sample.get("business_label_analysis"), dict) else {}
    evidence_block = analysis.get("evidence_block", {}) if analysis else {}
    if evidence_block:
        evidence.extend(str(item) for item in evidence_block.get("evidence", []))
        score = max(score, float(evidence_block.get("score", 0.0)))
        labels.extend(analysis.get("summary", {}).get("business_labels", []))

    virus_name = str(sample.get("virus_name", "")).lower()
    family = str(sample.get("fraud_family", "")).lower()
    haystack = f"{virus_name} {family}"
    if any(k in haystack for k in ("fraud", "phish", "scam", "loan")):
        labels.append("涉诈相关")
        score += 0.28
    if any(k in haystack for k in ("trojan", "spy", "stealer")):
        labels.append("恶意软件家族")
        score += 0.25
    if "rogue" in haystack:
        labels.append("流氓或欺诈推广")
        score += 0.16
    app_name = str(sample.get("app_name", "")).strip()
    if app_name:
        evidence.append(f"业务语义分析使用应用名称“{app_name}”。")

    if labels:
        evidence.append(f"推断出的业务标签：{', '.join(labels)}。")
    else:
        evidence.append("当前字段不足以推断明确业务标签。")

    missing = list(evidence_block.get("missing_fields", []))
    if not virus_name and not family:
        missing.extend(["virus_name", "fraud_family"])
    score = clamp(score)
    return EvidenceBlock("business_label", label_claim(score, "业务影响风险"), evidence, score, sorted(set(missing)), score)
...
def impersonation_agent(sample: dict[str, Any]) -> EvidenceBlock:
    ...
    for field in assessment.get("missing_fields", []) if isinstance(assessment.get("missing_fields"), list) else []:
...
def add_evidence_conflict_markers(blocks: list[EvidenceBlock]) -> list[EvidenceBlock]:
    ...
    for block in blocks:
        ...
        malicious_items = [item for item in concrete if item.get("direction") == "supports_malicious"]
...
def debate(evidence_blocks: list[EvidenceBlock], config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    avg = sum(block.score for block in evidence_blocks) / max(1, len(evidence_blocks))
    max_block = max(evidence_blocks, key=lambda item: item.score)
    min_block = min(evidence_blocks, key=lambda item: item.score)

    model_a_score = clamp(avg * 0.86 + min_block.score * 0.14)
    model_b_score = clamp(avg * 0.78 + max_block.score * 0.22)
    fallback_a_verdict = verdict_from_score(model_a_score)
    fallback_b_verdict = verdict_from_score(model_b_score)

    cross_examination = default_cross_examination()
    debate_rounds = int(os.getenv("MALAPP_DEBATE_ROUNDS", "1"))

    if local_qwen_enabled():
        llm_payload = json.dumps([asdict(block) for block in evidence_blocks], ensure_ascii=False, indent=2)
        model_a_raw = qwen_generate(
            "你是模型甲，角色是保守复核模型。你负责研判恶意 APP，但只有在多个证据块互相印证时才提高风险结论。"
            "必须只输出 JSON，不要输出 Markdown。",
            build_debate_prompt(llm_payload, "保守复核：重点检查证据是否充分、是否存在单点证据过度推断。"),
        )
        model_b_raw = qwen_generate(
            "你是模型乙，角色是风险优先模型。你负责研判恶意 APP，重点关注 IOC、仿冒、涉诈家族和高危权限。"
            "必须只输出 JSON，不要输出 Markdown。",
            build_debate_prompt(llm_payload, "风险优先：重点检查高危信号是否足以提升风险结论。"),
        )
        model_a_result = normalize_llm_result(model_a_raw, model_a_score, fallback_a_verdict)
        model_b_result = normalize_llm_result(model_b_raw, model_b_score, fallback_b_verdict)
        model_a_score = model_a_result["score"]
        model_b_score = model_b_result["score"]
        if debate_rounds >= 2:
            cross_examination = run_dynamic_debate_rounds(
                llm_payload,
                model_a_result,
                model_b_result,
        ...
        model_a_result = {
            "score": model_a_score,
            "verdict": fallback_a_verdict,
            "arguments": [
                "除非多个独立证据块形成一致支撑，否则倾向降低风险结论。",
                f"当前最弱证据维度为 {min_block.agent}，置信度 {min_block.confidence:.2f}。",
            ],
            "raw_text": "",
        }
        model_b_result = {
            "score": model_b_score,
            "verdict": fallback_b_verdict,
            "arguments": [
                "优先关注高影响 IOC、仿冒、涉诈家族等高风险证据。",
                f"当前最强证据维度为 {max_block.agent}，置信度 {max_block.confidence:.2f}。",
            ],
            "raw_text": "",
        }
    ...
    return {
        "model_a": {
            "role": "保守复核模型",
            "score": model_a_score,
            "verdict": model_a_result["verdict"],
            "verdict_label": verdict_label(model_a_score),
            "arguments": model_a_result["arguments"],
            "model_backend": "local_qwen2.5_0.5b" if local_qwen_enabled() else "rule_fallback",
            "raw_text": model_a_result.get("raw_text", ""),
        },
        "model_b": {
            "role": "风险优先模型",
            "score": model_b_score,
            "verdict": model_b_result["verdict"],
            "verdict_label": verdict_label(model_b_score),
            "arguments": model_b_result["arguments"],
            "model_backend": "local_qwen2.5_0.5b" if local_qwen_enabled() else "rule_fallback",
            "raw_text": model_b_result.get("raw_text", ""),
        },
        "cross_examination": cross_examination,
        "debate_rounds": debate_rounds if local_qwen_enabled() else 1,
        "arbiter": {
            "score": c_score,
            "verdict": verdict_from_score(c_score),
            "verdict_label": verdict_label(c_score),
            "rationale": "当前为占位仲裁器：综合保守模型、风险优先模型和最强证据块得出 Engine C 分数；后续可替换为真实大模型调用。",
        },
}


def default_cross_examination() -> list[dict[str, Any]]:
    return [
        {
            "round": 1,
            "type": "challenge",
            "from": "model_b",
            "to": "model_a",
            "question": "保守视角是否低估了最强证据块的风险？",
            "answer": "仲裁器会保留高置信证据，但要求其他维度提供辅助支撑。",
        },
        {
            "round": 1,
            "type": "challenge",
            "from": "model_a",
            "to": "model_b",
            "question": "风险优先视角是否过度依赖单一未验证信号？",
            "answer": "若高风险证据缺少交叉印证，仲裁器会下调整体置信度。",
        },
    ]


def run_dynamic_debate_rounds(
    evidence_json: str,
    ...
    *,
    ...
    """Run model-to-model challenge and rebuttal rounds.

    CPU inference is slow, so this is opt-in through MALAPP_DEBATE_ROUNDS.
    `max_rounds=2` means one challenge round and one rebuttal round.
    """
    rounds: list[dict[str, Any]] = []
    max_rounds = max(2, min(max_rounds, 3))

    challenge_b_to_a = qwen_generate(
        "你是模型乙，负责从风险优先视角质疑模型甲。只输出 JSON。",
        build_challenge_prompt(evidence_json, "model_a", model_a_result),
        max_new_tokens=180,
    )
    challenge_a_to_b = qwen_generate(
        "你是模型甲，负责从保守复核视角质疑模型乙。只输出 JSON。",
        build_challenge_prompt(evidence_json, "model_b", model_b_result),
        max_new_tokens=180,
    )
    challenge_b = normalize_debate_message(challenge_b_to_a)
    challenge_a = normalize_debate_message(challenge_a_to_b)
    rounds.extend(
        [
            {
                "round": 1,
                ...
                "from": "model_b",
                "to": "model_a",
                "question": challenge_b["question"],
                "answer": challenge_b["answer"],
                "raw_text": challenge_b_to_a,
            },
            {
                "round": 1,
                "type": "challenge",
                "from": "model_a",
                "to": "model_b",
                "question": challenge_a["question"],
                "answer": challenge_a["answer"],
                "raw_text": challenge_a_to_b,
            },
        ]
    )

    if max_rounds >= 2:
        rebuttal_a_raw = qwen_generate(
            "你是模型甲，请针对模型乙的质疑进行辩护性反驳。只输出 JSON。",
            build_rebuttal_prompt(evidence_json, model_a_result, challenge_b),
            max_new_tokens=180,
        )
        rebuttal_b_raw = qwen_generate(
            "你是模型乙，请针对模型甲的质疑进行辩护性反驳。只输出 JSON。",
            build_rebuttal_prompt(evidence_json, model_b_result, challenge_a),
            max_new_tokens=180,
        )
        rebuttal_a = normalize_debate_message(rebuttal_a_raw)
        rebuttal_b = normalize_debate_message(rebuttal_b_raw)
        rounds.extend(
            [
                {
                    "round": 2,
                    ...
                    "to": "model_b",
                    "question": challenge_b["question"],
                    "answer": rebuttal_a["answer"],
                    "raw_text": rebuttal_a_raw,
                },
                {
                    "round": 2,
                    "type": "rebuttal",
                    ...
                    "to": "model_a",
                    "question": challenge_a["question"],
                    "answer": rebuttal_b["answer"],
                    "raw_text": rebuttal_b_raw,
                },
            ]
        )
...
def build_challenge_prompt(evidence_json: str, target_name: str, target_result: dict[str, Any]) -> str:
    return f"""
证据块：
{evidence_json}

对方初判：
{json.dumps(target_result, ensure_ascii=False)}

请指出 {target_name} 的一个主要逻辑漏洞、证据遗漏或过度推断。
只输出 JSON：
{{
  "question": "对方哪里可能有问题？",
  "answer": "你的质疑理由，必须引用具体证据"
}}
""".strip()


def build_rebuttal_prompt(evidence_json: str, own_result: dict[str, Any], challenge: dict[str, str]) -> str:
    return f"""
证据块：
{evidence_json}

你的初判：
{json.dumps(own_result, ensure_ascii=False)}

对方质疑：
{json.dumps(challenge, ensure_ascii=False)}

请对质疑进行反驳、承认修正或补充证据。只输出 JSON：
{{
  "question": "你回应的是哪条质疑？",
  "answer": "你的反驳或修正，必须引用具体证据"
}}
""".strip()


def normalize_debate_message(raw_text: str) -> dict[str, str]:
    data = parse_model_json(raw_text)
    question = str(data.get("question") or data.get("质疑") or data.get("问题") or "").strip()
    answer = str(data.get("answer") or data.get("理由") or data.get("response") or "").strip()
    if not question:
        question = "未能解析出明确质疑。"
    if not answer:
        answer = raw_text.strip()[:300] or "模型未生成有效回应。"
    return {"question": question, "answer": answer}


def build_debate_prompt(evidence_json: str, strategy: str) -> str:
    return f"""
请根据以下四个专家智能体证据块，对样本进行一次独立研判。
...
请只输出一个 JSON 对象，格式必须如下：
{{
  "verdict": "malicious | suspicious | benign",
  "score": 0.0,
  "arguments": ["理由1", "理由2", "理由3"]
}}

要求：
1. score 是 0 到 1 的恶意倾向分。
2. verdict 必须和 score 基本一致：>=0.85 为 malicious，0.60-0.85 为 suspicious，<0.60 为 benign。
3. arguments 必须引用具体证据块，不要泛泛而谈。
""".strip()


def wec_decision(sample: dict[str, Any], c_score: float, evidence_blocks: list[EvidenceBlock]) -> dict[str, Any]:
    score_a = float(sample.get("engine_a_score", 50)) / 100
    score_b = float(sample.get("engine_b_score", 50)) / 100
    weights = {"engine_a": 1.0, "engine_b": 1.0, "engine_c": 1.0}

    strong_agents = [block.agent for block in evidence_blocks if block.score >= 0.7]
    if "threat_intel" in strong_agents:
        weights["engine_c"] += 0.45
        weights["engine_a"] = max(0.4, weights["engine_a"] - 0.2)
        weights["engine_b"] = max(0.4, weights["engine_b"] - 0.2)
    if "impersonation" in strong_agents:
        weights["engine_c"] += 0.25
    if abs(score_a - score_b) >= 0.35:
        weights["engine_c"] += 0.2
    if sample.get("static_trust", {}).get("level") == "low":
        weights["engine_c"] += 0.25

    final_score = (
        score_a * weights["engine_a"] + score_b * weights["engine_b"] + c_score * weights["engine_c"]
    ) / sum(weights.values())
    final_score = clamp(final_score)
    return {
        "final_score": final_score,
        "verdict": verdict_from_score(final_score),
        "verdict_label": verdict_label(final_score),
        "risk_level": risk_level(final_score),
        "risk_level_label": RISK_LEVEL_LABELS[risk_level(final_score)],
        "weights": weights,
        "engine_scores": {
            "engine_a": score_a,
            "engine_b": score_b,
...
def verdict_label(score: float) -> str:
    return VERDICT_LABELS[verdict_from_score(score)]
...
def judge(raw_sample: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        from engine.xgb_runtime import enrich_sample as enrich_xgb_sample
    ...
    if use_hermes:
        from engine.hermes_runtime import run_hermes_supervisor
    ...
    xgb_result = None
    try:
        from engine.xgb_runtime import predict as predict_xgb
    ...
    try:
        from engine.agent_trace import save_agent_trace
    ...
    try:
        from engine.reward_builder import save_reward_for_report
