...
def build_rag_retrieval_scorecard(dataset_jsonl: Path) -> dict[str, Any]:
    ...
    mean = lambda values: round(statistics.fmean(values), 6) if values else None
...
def capture_runtime_snapshot(data_dir: Path | None = None) -> dict[str, Any]:
    ...
    code_files = [
        ROOT / "app_version.py",
        ROOT / "engine" / "pipeline.py",
        ROOT / "engine" / "debate_flow.py",
        ROOT / "engine" / "rag" / "store.py",
        ROOT / "engine" / "decision_engine.py",
    ]
    code_hashes = {
