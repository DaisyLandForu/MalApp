...
WHITE_DIR = DATA_ROOT / "benign"
OUTPUT_DIR = ROOT / "training_artifacts"
DB_PATH = OUTPUT_DIR / "training_dataset.db"
SCHEMA_PATH = ROOT / "training" / "schemas" / "evidence.schema.json"
...
def fit_binary_model(
    ...
    return model, {
        ...
        "top_weights": sorted(
            zip(feature_names, weights.tolist(), strict=False), key=lambda item: abs(item[1]), reverse=True
        )[:20],
    }
...
def evaluate_model(model: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    tp = sum(p == y == 1 for p, y in zip(preds, labels, strict=False))
    tn = sum(p == y == 0 for p, y in zip(preds, labels, strict=False))
    fp = sum(p == 1 and y == 0 for p, y in zip(preds, labels, strict=False))
    fn = sum(p == 0 and y == 1 for p, y in zip(preds, labels, strict=False))
...
def brier(probabilities: list[float], labels: list[int]) -> float:
    return statistics.fmean((p - y) ** 2 for p, y in zip(probabilities, labels, strict=False))


def log_loss(probabilities: list[float], labels: list[int]) -> float:
    return statistics.fmean(
        -(y * math.log(max(p, 1e-9)) + (1 - y) * math.log(max(1 - p, 1e-9)))
        for p, y in zip(probabilities, labels, strict=False)
    )
...
def roc_auc(probabilities: list[float], labels: list[int]) -> float:
    ...
    if positives == 0 or negatives == 0:
        return 0.5
    ordered = sorted(zip(probabilities, labels, strict=False), key=lambda item: item[0])
    rank_sum = 0.0
    index = 0
