...
文件：`engine/evaluation_framework.py`
...
```powershell
python tools\evaluation_cli.py freeze --name v1
```
...
```powershell
python tools\evaluation_cli.py run `
  --variant full `
...
```powershell
python tools\evaluation_cli.py scorecard `
...
```powershell
python tools\evaluation_cli.py datasets `
  --core-size 500 `
  --challenge-size 300 `
  --rag-size 200
```
...
```powershell
python tools\evaluation_cli.py rag-scorecard `
...
```powershell
python tools\evaluation_cli.py run --variant full --run-id ablation-full --limit 500

python tools\evaluation_cli.py run --variant full --run-id ablation-no-static `
  --disabled-agent static_analysis --limit 500

python tools\evaluation_cli.py run --variant full --run-id ablation-no-threat `
  --disabled-agent threat_intel --limit 500

python tools\evaluation_cli.py run --variant full --run-id ablation-no-impersonation `
  --disabled-agent impersonation --limit 500

python tools\evaluation_cli.py run --variant full --run-id ablation-no-business `
...
```powershell
python tools\evaluation_cli.py run --variant rag_off --run-id rag-off --limit 500
python tools\evaluation_cli.py run --variant rag_vector --run-id rag-vector --limit 500
python tools\evaluation_cli.py run --variant rag_hybrid --run-id rag-hybrid --limit 500
```

### 5.3 完整辩论与快速验证

```powershell
python tools\evaluation_cli.py run --variant full --run-id debate-full --limit 500
python tools\evaluation_cli.py run --variant verification --run-id debate-short --limit 500
```
