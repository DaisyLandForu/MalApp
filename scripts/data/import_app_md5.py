...
from malapp.application.judgement import DB_PATH, init_db
...
BASIC_INFO_URL = os.getenv("MALAPP_BASIC_INFO_URL_TEMPLATE", "").strip()
URL_INFO_URL = os.getenv("MALAPP_URL_INFO_URL_TEMPLATE", "").strip()
...
def fetch_app_api(md5: str, fields: str = DEFAULT_FIELDS, timeout: float = 8.0) -> dict[str, Any]:
    if not BASIC_INFO_URL or not URL_INFO_URL:
        raise RuntimeError(
            "set MALAPP_BASIC_INFO_URL_TEMPLATE and MALAPP_URL_INFO_URL_TEMPLATE before enrichment"
        )
    md5 = md5.upper().strip()
