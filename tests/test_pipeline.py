...
from engine.debate_flow import compact_evidence_for_llm, coerce_initial_schema_values, normalize_turn, placeholder_model_output, valid_closing_schema, valid_initial_schema
from engine.llm_local import parse_model_json
from engine.decision_engine import collaborative_decision, debate_confidence
from engine.preprocess import merge_network_packages, normalize_feature_record
from engine.threat_intelligence import extract_network_indicators
...
class PipelineTest(unittest.TestCase):
    ...
    def test_high_confidence_xgb_uses_full_debate_by_default(self):
        ...
        with (
            patch("engine.pipeline.local_qwen_enabled", return_value=True),
            patch("engine.preprocess.get_cached_report", return_value=None),
            patch("engine.xgb_runtime.predict", return_value=xgb_result),
            patch("engine.pipeline.debate", return_value=debate_result) as debate_mock,
        ):
            report = judge(
                {
        ...
        self.assertEqual(report["decision"]["verdict"], "malicious")
        self.assertEqual(
