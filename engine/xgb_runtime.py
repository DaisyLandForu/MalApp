...
try:
    from ml_pipeline import xgb_pipeline as training
...
DB_PATH = RUNTIME_DIR / "runtime_training.db"
if not DB_PATH.exists():
    DB_PATH = RUNTIME_DIR / "xgb_training.db"
...
def load_runtime() -> dict[str, Any]:
    if xgb is None or training is None:
        ...
        if training is None:
            details.append(f"ml_pipeline.xgb_pipeline={TRAINING_IMPORT_ERROR or 'not imported'}")
    ...
    return {"manifest": manifest, "models": models, "references": references}
...
def enrich_sample(sample: dict[str, Any]) -> dict[str, Any]:
    ...
    if not md5 or not DB_PATH.exists():
        return result
    with closing(sqlite3.connect(DB_PATH)) as conn:
        row = conn.execute(
