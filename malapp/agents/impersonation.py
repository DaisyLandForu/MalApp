...
from malapp.config.paths import PROJECT_ROOT, resolve_data_dir, resolve_workspace_dir
...
def compare_hashes(left: str, right: str) -> float:
    ...
    if len(left) == len(right) and all(ch in "0123456789abcdef" for ch in left + right):
        distance = sum(1 for a, b in zip(left, right, strict=False) if a != b)
...
def compare_images(left: bytes, right: bytes) -> float:
    ...
    try:
        ...
        mse = sum(
            ...
            for lp, rp in zip(left_pixels, right_pixels, strict=False)
        ) / (len(left_pixels) * 3)
...
def has_transposition(left: str, right: str) -> bool:
    if len(left) != len(right):
        return False
    diffs = [idx for idx, (a, b) in enumerate(zip(left, right, strict=False)) if a != b]
