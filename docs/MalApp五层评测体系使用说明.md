...
- `tools/five_layer_eval_cli.py`：生成、校验、模型打分、RAG 打分和漂移检测。
...
```powershell
python tools\five_layer_eval_cli.py build-rag-corpus --size 2000
```
...
```powershell
python tools\five_layer_eval_cli.py generate --name v1
python tools\five_layer_eval_cli.py validate "<套件目录>"
python tools\five_layer_eval_cli.py overview
```
...
```powershell
python tools\five_layer_eval_cli.py score-model `
...
```powershell
python tools\five_layer_eval_cli.py score-rag `
...
```powershell
python tools\evaluation_cli.py run --variant full --run-id ablation-full --limit 300
python tools\evaluation_cli.py run --variant full --run-id ablation-no-static --disabled-agent static_analysis --limit 300
python tools\evaluation_cli.py run --variant full --run-id ablation-no-threat --disabled-agent threat_intel --limit 300
python tools\evaluation_cli.py run --variant full --run-id ablation-no-impersonation --disabled-agent impersonation --limit 300
python tools\evaluation_cli.py run --variant full --run-id ablation-no-business --disabled-agent business_label --limit 300
```
...
```powershell
python tools\five_layer_eval_cli.py drift `
  "<套件目录>" `
  "<当前批次CSV>" `
  --output "<漂移报告.json>"
```
