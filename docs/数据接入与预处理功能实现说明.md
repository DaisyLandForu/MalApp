...
本文档对应“数据接入与预处理”功能点，说明 `test1` 当前已经实现的功能、对应代码位置和实现边界。
...
```text
engine/import_feature_workbooks.py
```
...
.\.venv\Scripts\python.exe -m engine.import_feature_workbooks
```

单独导入 APP 研判字段：

```powershell
.\.venv\Scripts\python.exe -m engine.import_feature_workbooks --only app
```

单独导入人工标注冲突表：

```powershell
.\.venv\Scripts\python.exe -m engine.import_feature_workbooks --only conflict
```
